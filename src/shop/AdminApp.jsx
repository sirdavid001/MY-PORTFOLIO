...
function normalizeOrderItems(items) {
  ...
  return raw
    .map((item, index) => {
      ...
      const unitPrice = Number.isFinite(unitPriceNumber) && unitPriceNumber >= 0 ? unitPriceNumber : null;
      return {
                    ...
                    return (
                      <article key={order.id} className="px-4 py-4 sm:px-5">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <span
                              className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${orderStatusBadgeClass(
                                order.status
                              )}`}
                            >
                        ...
                        <div className="mt-4 grid gap-3 xl:grid-cols-[1.6fr_1fr]">
                          ...
                          <div className="space-y-3">
                            <section className="rounded-2xl border border-slate-200 bg-white p-3">
                              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">Customer Details</p>
                            ...
                            <section className="rounded-2xl border border-slate-200 bg-white p-3">
                              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">Payment & Totals</p>
                              <p className="mt-2 text-xs text-slate-600">
                                Method: {String(order.payment_method || "Paystack").trim() || "Paystack"}
                              </p>
                              {hasBreakdown ? (
                                <>
                                  <p className="mt-1 text-xs text-slate-600">Subtotal: {formatMoney(subtotal, currency)}</p>
                                  <p className="mt-1 text-xs text-slate-600">Shipping: {formatMoney(shippingFee, currency)}</p>
                                </>
                              ) : null}
                              <p className="mt-1 text-sm font-semibold text-slate-900">
